...
if user_menu == 'Overall Analysis':
    ...
    # 'Edition' corresponds to 'Year', 'region' corresponds to 'col'
    st.title("Participating Nations over the years")
    st.plotly_chart(fig)

    events_over_time = help.data_over_time(df, 'Event')
    fig = px.line(events_over_time, x="Edition", y="count")
    st.title("Events over the years")
    st.plotly_chart(fig)

    athlete_over_time = help.data_over_time(df, 'Name')
...
if user_menu == 'Athlete wise Analysis':
    ...
    selected_sport = st.selectbox('Select a Sport', sport_list)
    df = help.weight_v_height(df, selected_sport)

    # Separate the x and y coordinates
    x = df['Weight']
    y = df['Height']

    # Use scatterplot with the x and y arguments
    fig, ax = plt.subplots()
    ax = sns.scatterplot(x=x, y=y, hue=df['Medal'], style=df['Sex'], s=60)
    st.pyplot(fig)
